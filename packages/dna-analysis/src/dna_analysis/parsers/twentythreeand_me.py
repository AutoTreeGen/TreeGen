"""Парсер 23andMe v5 raw данных.

Формат документирован 23andMe в header'е каждого экспорта:
    # This data file generated by 23andMe at: ...
    # rsid\tchromosome\tposition\tgenotype
    rs4477212\t1\t82154\tAA
    ...

Reference build для v5 — GRCh37 (явно указано в header'е). Версии v4
и старше использовали другие сборки; этот парсер — только под v5.
...
# Сигнатура header'а 23andMe v5. Ищем в первых ~20 строках, чтобы
# detect() оставался O(1) по размеру файла.
_DETECT_SIGNATURE: Final = "# This data file generated by 23andMe"
_DETECT_HEAD_LINES: Final = 20

# 23andMe v5 — GRCh37 / hg19. Версия пишется в header, но фиксирована
# для всего v5-режима экспорта.
...
# Все валидные сырые токены genotype от 23andMe → нормализованный Genotype.
# Гетерозиготы упорядочены лексикографически (AC, не CA). 23andMe не
# гарантирует порядок аллелей, поэтому строим таблицу обоих перестановок.
...
class TwentyThreeAndMeParser(BaseDnaParser):
    """Парсер 23andMe v5 raw TSV (GRCh37, 4 columns)."""
    ...
    def parse(self, content: str) -> DnaTest:
        if not self.detect(content):
            msg = "content does not look like a 23andMe v5 raw file"
        ...
        _LOG.debug("parsed %d SNPs from 23andMe file [%s]", len(snps), file_hash)
...
def _parse_snp_row(line: str, line_number: int) -> dict[str, object]:
    """Парсит одну SNP-строку 23andMe TSV в kwargs для Snp(...).

    Возвращает dict, чтобы Pydantic-валидация на уровне DnaTest(snps=[...])
    собрала всё в один проход и выдала единый ValidationError при необходимости
    (но в практике мы валидируем поля сами здесь, чтобы выдать DnaParseError
    с line number вместо ValidationError без контекста).
    """
    parts = line.split("\t")
