"""Async engine + session factory для archive-service.

Engine создаётся один раз при старте (lifespan); сессии — per-request
через FastAPI ``Depends(get_session)``. Mirrors parser-service.

Phase 15.5: первая DB-зависимость в archive-service. До неё сервис был
read-only прокси к FamilySearch без локального стейта.
"""
