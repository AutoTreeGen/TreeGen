"""Async engine + session factory для notification-service.

Зеркалит ``dna-service/database.py``: один engine на процесс,
audit-listeners регистрируются один раз. Если auth появится позже,
часть лог-логики переедет сюда; пока — простой proxy.
"""
