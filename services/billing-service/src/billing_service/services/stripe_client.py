...
def construct_webhook_event(
    ...
    """Верифицировать подпись и вернуть распарсенный event.

    Бросает ``stripe.error.SignatureVerificationError`` (= наследник
    ValueError-подобного), если подпись неверна или payload изменён.
    Caller (webhook handler) ловит исключение и возвращает 400.

    Возвращаемый объект — Stripe Event (Pydantic-ish), мы кастуем
    к dict для удобства serialization в БД.
    """
    ...
    event = stripe.Webhook.construct_event(  # type: ignore[no-untyped-call,unused-ignore]
