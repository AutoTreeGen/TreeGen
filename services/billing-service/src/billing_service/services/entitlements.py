"""Резолюция плана пользователя + feature-gating (Phase 12.0).

См. ADR-0034 §«Plan limits» для бизнес-смысла лимитов.

Ключевые функции:

* ``get_user_plan(session, user_id) -> Plan`` — async-резолвер плана
  из ``stripe_subscriptions``. Применяет grace-period для PAST_DUE.
...
from shared_models.orm import StripeSubscription
...
class PlanLimits:
    ...
    ``None`` означает «без лимита» (PRO-режим).
...
async def get_user_plan(session: AsyncSession, user_id: uuid.UUID) -> Plan:
    ...
    1. ``ACTIVE`` подписка → возвращаем её ``plan`` (обычно PRO).
    2. ``PAST_DUE`` в окне grace-period (см. ADR-0034 §«Failed payment») →
       продолжаем давать доступ. Окно — ``settings.past_due_grace_days``
       дней с момента ``current_period_end``.
    3. Всё остальное (CANCELED, INCOMPLETE, нет записи) → FREE.

    Эта функция — единая точка правды для feature-gating'а. Любое
    усложнение бизнес-правил (team plans, quotas-на-периодные-кредиты)
    должно приходить сюда.
    """
    settings = get_settings()
    if not settings.billing_enabled:
        # Local-dev / CI: feature flag отключает биллинг → все юзеры на PRO.
        # См. ADR-0034 §«Feature flag».
        return Plan.PRO
    ...
    if status_value == SubscriptionStatus.PAST_DUE.value:
        if sub.current_period_end is None:
            ...
            # обращаемся как с FREE, чтобы не давать вечный доступ.
            return Plan.FREE
...
def resolve_user_id_from_header(x_user_id: str | None) -> uuid.UUID:
    ...
    Экспортируется как public-helper, чтобы сервисы переиспользовали
    тот же error-format/parsing — иначе каждый сервис писал бы свою
    проверку с разными 401 detail'ями.

    Header должен содержать UUIDv7 (тот же формат, что у ``users.id``).
    """
...
def feature_allowed(feature: Feature, limits: PlanLimits) -> bool:
    ...
    ``import_quota`` всегда разрешён булево — реальный per-tier лимит
    проверяется отдельно в endpoint'е (через count-запрос). Здесь только
    «может ли план в принципе импортировать».
    """
...
async def assert_feature(
    ...
    Бросает 402 Payment Required при отсутствии доступа.
    Используется в caller-сервисах внутри тонкого FastAPI dependency:

    .. code-block:: python

        async def require_dna(
            ...
            await assert_feature(session, user_id, "dna_enabled")

    Сами числовые квоты (``max_trees``, ``max_persons_per_tree``) эта
    функция **не** проверяет — они требуют count-запросов к доменным
    таблицам и должны быть встроены в конкретные endpoint'ы. Здесь —
    только булевые feature-флаги.
    """
...
def check_entitlement(feature: Feature) -> object:
    """Фабрика FastAPI dependency'и для feature-gating'а **внутри billing-service**.

    Использует ``get_session`` из billing-service. Для caller-сервисов
    (parser-service, dna-service) предпочтительнее писать свой тонкий
    dependency через ``assert_feature`` — он использует session из
    своего собственного engine.
    """
    ...
    async def _dep(
        ...
        if not settings.billing_enabled:
            return  # feature-flag mode: пропускаем все проверки.
        user_id = resolve_user_id_from_header(x_user_id)
