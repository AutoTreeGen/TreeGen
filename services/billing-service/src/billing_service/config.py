...
class Settings(BaseSettings):
    ...
    stripe_price_free: str = Field(
        default="",
        description="price_* — Stripe Price ID Free-плана (placeholder для completeness).",
    )
    ...
    stripe_price_premium: str = Field(
        default="",
        description="price_* — Stripe Price ID Premium-плана (Phase 12.x).",
    )
    ...
    email_service_url: str = Field(
        ...
        description=(
            "Base URL email-service. Используется для POST /email/send "
            "из invoice.* webhook handler'ов (kind=payment_succeeded|payment_failed). "
            "Пустая строка отключает email fan-out."
        ),
    )
    ...
    past_due_grace_days: int = Field(
        ...
        description=(
            ...
            "доступ к фичам Pro. См. ADR-0042 §«Failed payment policy»."
        ),
    )
